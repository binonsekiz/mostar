...
import geometry.libgdxmath.Plane.PlaneSide;
import geometry.libgdxmath.collision.BoundingBox;
...
public final class Intersector {
	...
	public static boolean isPointInPolygon (float[] polygon, Vector2 point) {
		...
		for (int i = 0; i < polygon.length/2; i++) {
			float verticex = polygon[2 * i];
			float verticey = polygon[2 * i + 1];
			if (verticey < point.y && lastVerticey >= point.y || lastVerticey < point.y && verticey >= point.y) {
				if (verticex + (point.y - verticey) / (lastVerticey - verticey) * (lastVerticex - verticex) < point.x) {
					oddNodes = !oddNodes;
				}
			}
			lastVerticex = verticex;
			lastVerticey = verticey;
		}
		return oddNodes;
	}
	...
	public static Vector2 closestPointOnLineSegment (LineSegment segment, Vector2 point) {
		...
		float t = tmp.dot(tmp2) / l2;
		if (t < 0.0f)
			return start; // Beyond 'start'-end of the segment
		else if (t > 1.0f) return end; // Beyond 'end'-end of the segment
	...
	public static boolean intersectRayTriangle (Ray ray, Vector3 t1, Vector3 t2, Vector3 t3, Vector3 intersection) {
		...
		System.out.println("++Intersect Ray Triangle2");
		v0.set(t3).sub(t1);
		v1.set(t2).sub(t1);
		v2.set(i).sub(t1);
		...
		System.out.println("++Intersect Ray Triangle3");
		float u = (dot11 * dot02 - dot01 * dot12) / denom;
		float v = (dot00 * dot12 - dot01 * dot02) / denom;
		
		System.out.println("++Intersect Ray Triangle4");
		if (u >= 0 && v >= 0 && u + v <= 1) {
	...
	public static boolean intersectRayTriangles (Ray ray, float[] vertices, short[] indices, int vertexSize, Vector3 intersection) {
		...
		for (int i = 0; i < indices.length; i += 3) {
			...
			if (vertexSize >= 3)
			{
				...
				result = intersectRayTriangle(ray, tmp1.set(vertices[i1], vertices[i1 + 1], 0),
						tmp2.set(vertices[i2], vertices[i2 + 1], 0),
						tmp3.set(vertices[i3], vertices[i3 + 1], 0), tmp);
				System.out.println("checking triangle: " + tmp1 + ", " + tmp2 + ", " + tmp3);
			}
			else {
				throw new RuntimeException("invalid vertex size");
			}
			if (result == true) {
				float dist = ray.origin.tmp().sub(tmp).len2();
				if (dist < min_dist) {
